...
@section('content')
    <div class="login-main" style="background-image: url('{{ asset('assets/user/images/login.jpg') }}')">
        <div class="container custom-container d-flex justify-content-center">
            <div class="login-area">
                ...
                <form action="{{ route('user.password.verify.code') }}" method="POST" class="login-form w-100">
                    @csrf
                    ...
                    <div class="d-flex flex-wrap justify-content-between">
                        <a href="{{ route('user.password.reset') }}" class="forget-text">@lang('Try to send again')</a>
                    </div>
                    <button type="submit" class="btn cmn-btn w-100 mt-4">@lang('Submit')</button>
                </form>
                <a href="{{ route('user.login') }}" class="text-white mt-4"><i class="las la-sign-in-alt" aria-hidden="true"></i>@lang('Back to Login')</a>
            </div>
        </div>
    </div>
...
@push('style')
    <link rel="stylesheet" href="{{ asset('assets/user/css/verification_code.css') }}">
@endpush
